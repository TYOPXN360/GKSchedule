...
import com.ty.gkschedule.data.CourseDao
...
class ScheduleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val examDao: com.ty.gkschedule.data.ExamDao = CourseDatabase.getDatabase(application).examDao()
    ...
    val showHiddenCourses: Flow<Boolean> = settings.showHiddenCourses
    ...
    val autoCheckUpdateDaily: Flow<Boolean> = settings.autoCheckUpdateDaily
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            com.ty.gkschedule.sync.AutoSyncWorker.schedule(app, value, unit)
        }
        // Schedule token heartbeat if enabled
        viewModelScope.launch {
            val heartbeatEnabled = settings.tokenHeartbeat.first()
            if (heartbeatEnabled) {
                com.ty.gkschedule.sync.HeartbeatWorker.schedule(app)
            }
        }
    }
    ...
    fun setTokenHeartbeat(enabled: Boolean) {
        viewModelScope.launch {
            ...
            if (enabled) {
                ...
                com.ty.gkschedule.sync.HeartbeatWorker.cancel(app)
            }
        }
    }
    ...
    fun setShowHiddenCourses(show: Boolean) {
        viewModelScope.launch { settings.setShowHiddenCourses(show) }
    }

    fun setCompactNavBar(compact: Boolean) {
        viewModelScope.launch { settings.setCompactNavBar(compact) }
    }

    fun setPillContentMode(mode: Int) {
        viewModelScope.launch { settings.setPillContentMode(mode) }
    }
    ...
    fun setGoSignWeeklyEnabled(enabled: Boolean) {
        viewModelScope.launch { settings.setGoSignWeeklyEnabled(enabled) }
    }
    ...
    private suspend fun importFromSchool(jobNumber: String) {
        try {
            ...
            val allRemoteCourses = mutableListOf<com.ty.gkschedule.api.RemoteCourse>()
            for (week in 1..weeks) {
                try {
    ...
    fun refreshFromSchool() {
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                val remoteCourses = mutableListOf<com.ty.gkschedule.api.RemoteCourse>()
                var failedWeeks = 0
                for (week in 1..weeks) {
                    try {
            ...
            try {
                ...
                result.onSuccess { exams ->
                    ...
                    val entities = exams.map { e ->
                        com.ty.gkschedule.data.ExamEntity(
                            courseName = e.kcmc,
    ...
    fun saveExams(exams: List<com.ty.gkschedule.data.ExamEntity>) {
