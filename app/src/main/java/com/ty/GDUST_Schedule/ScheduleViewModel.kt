...
import com.ty.GDUST_Schedule.api.CourseImporter
import com.ty.GDUST_Schedule.api.GdustApi
import com.ty.GDUST_Schedule.data.Course
import com.ty.GDUST_Schedule.data.CourseDao
import com.ty.GDUST_Schedule.data.CourseDatabase
import com.ty.GDUST_Schedule.data.CredentialStore
import com.ty.GDUST_Schedule.data.ExamEntity
import com.ty.GDUST_Schedule.data.SettingsDataStore
import com.ty.GDUST_Schedule.notification.ReminderScheduler
import com.ty.GDUST_Schedule.util.IcsExport
import com.ty.GDUST_Schedule.util.ImageExport
import com.ty.GDUST_Schedule.util.JsonImportExport
...
class ScheduleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val examDao: com.ty.GDUST_Schedule.data.ExamDao = CourseDatabase.getDatabase(application).examDao()
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            com.ty.GDUST_Schedule.sync.AutoSyncWorker.schedule(app, value, unit)
        }
        // Schedule token heartbeat if enabled
        viewModelScope.launch {
            val heartbeatEnabled = settings.tokenHeartbeat.first()
            if (heartbeatEnabled) {
                com.ty.GDUST_Schedule.sync.HeartbeatWorker.schedule(app)
            }
        }
    }
    ...
    fun setTokenHeartbeat(enabled: Boolean) {
        viewModelScope.launch {
            ...
            if (enabled) {
                ...
                com.ty.GDUST_Schedule.sync.HeartbeatWorker.cancel(app)
            }
        }
    }
    ...
    private suspend fun importFromSchool(jobNumber: String) {
        try {
            ...
            val allRemoteCourses = mutableListOf<com.ty.GDUST_Schedule.api.RemoteCourse>()
            for (week in 1..weeks) {
                try {
    ...
    fun refreshFromSchool() {
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                val remoteCourses = mutableListOf<com.ty.GDUST_Schedule.api.RemoteCourse>()
                var failedWeeks = 0
                for (week in 1..weeks) {
                    try {
            ...
            try {
                ...
                result.onSuccess { exams ->
                    ...
                    val entities = exams.map { e ->
                        com.ty.GDUST_Schedule.data.ExamEntity(
                            courseName = e.kcmc,
    ...
    fun saveExams(exams: List<com.ty.GDUST_Schedule.data.ExamEntity>) {
